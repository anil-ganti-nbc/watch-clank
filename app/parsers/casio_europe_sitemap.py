"""Parser for the Casio Europe sitemap-delta collector.

See app/collectors/casio_europe_sitemap.py for the full reconnaissance and
the honest limitation this parser is built around: the sitemap carries
only a canonical URL, a reference (extracted from the URL path, e.g.
`product.GBA-950-1A/` -> `GBA-950-1A`), and a `<lastmod>` timestamp --
never price, currency, or availability. Every ParsedWatch from this parser
has those three fields as None, deliberately, rather than guessed.
Byte-for-byte identical logic to casio_uk_sitemap_item; kept separate to
match this project's one-parser-per-region convention.
"""
...
PARSER_ID = "casio_europe_sitemap_item"
PARSER_VERSION = "0.1.0"


def parse_casio_europe_sitemap_item(payload: bytes | str | dict, *, source_url: str = "") -> ParseResult:
