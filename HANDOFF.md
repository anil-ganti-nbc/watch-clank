...
## 2026-09-10 — casio_multi silent component degradation: JP component swapped to the official sitemap lane

Operator reported casio_multi FAILED + PARTIAL-with-Fail-0 runs + the Casio Japan problem absent from logs. Forensics-first verdict (live DB, snapshot evidence, live probes): **Casio Japan HTML collection never worked from this vantage** — casio_multi run #1 (2026-08-10) was already Akamai-403 on all five discovery URLs; `source_component_states` shows `consecutive_blocks=33`, `last_success_at=NULL`, zero snapshots, zero products ever; live reprobe 2026-09-10 still 403s every listing page while `casio_jp_sitemap` (watches.xml) is healthy. The intermittent PARTIALs were the daily honest backoff-expiry probes; the "SUCCESS" runs were the degraded ones (component silently BACKED_OFF — treated as healthy by the combined-status rule and logging nothing); "Fail 0" is correct because `failure_count` is item-level and component state lived only in summary_metadata JSON; the log silence was the backoff skip path logging nothing. No FAILED casio_multi run exists in the canonical DB — the operator's manual FAILED attempt is not production state. Git window Aug-29..Sep-1 contains no relevant code changes.

Repair (operator option C): **`run_multi_source_pipeline`'s JP product component is now the official `CasioJPSitemapCollector` sitemap-delta lane** — new-first discovery against the JP lane's observation history (unchanged catalogue = healthy silent ZERO_ITEMS), only genuinely new references processed, same parser/region/attribution/emit-notify-baseline threading; legacy `CasioJapanCollector` code + tests untouched. Observability: one terminal structured record per component exit (`casio_component_finished ... http_statuses=[...]` — including the BACKED_OFF skip path), plus persisted `component_failures`/`component_skipped`/`status_reason` on the run row, and a Components column (run history) + Components card (run detail) in the UI. `failure_count` semantics unchanged (item-level). Full writeup: `ai/handoff/INCIDENT_CASIO_MULTI_SILENT_COMPONENT_DEGRADATION.md`. Tests: new `tests/test_casio_multi_component_health.py` + updated component mocks in test_core/test_novelty_semantics (blocked⇒PARTIAL persisted + logged; BACKED_OFF visible; unchanged catalogue never a failure; new JP ref parsed with NO false NEW_REFERENCE; parse regression ⇒ FAILED(PARSE_FAILURES)). No novelty/baseline semantics touched; no bypass attempted.

## 2026-09-08/09 — Horology Sentinel: fast first-party tripwire (implemented, not deployed)

Operator brief: watch news is competitive; we sometimes discover genuinely new Timex/Casio/Seiko/Citizen SKUs only after another writer claimed them. Built **Horology Sentinel** — a cheap, deterministic, first-stage watcher that polls first-party discovery surfaces every 15 minutes and pings Discord **WATCH SIGHTING** immediately for genuinely unseen identities, while the existing pipeline remains solely responsible for enrichment/novelty/editorial judgement. A sighting claims exactly one thing: a previously unseen identity appeared in a monitored source — `first_seen != market novelty` preserved by construction (the Sentinel writes no watches/observations/events).

Key design decisions, all evidence-based:
- **Sources (6, declarative in `app/sentinel/config.py`):** timex_us, timex_uk, seiko_us via the plain unscoped `/products.json` (live-verified newest-first on all three stores 2026-09-08; the collection-scoped variant is NOT reliably ordered and was rejected), casio_uk_sitemap + casio_jp_sitemap (delegating to those collectors' own proven `discover_from_sitemap_xml`, so URL/option/reference rules cannot drift), citizen_eu_sitemap (the only first-party machine-readable Citizen surface from this vantage; citizenwatch.co.uk stays untouched — robots `ClaudeBot: Disallow /` + Cloudflare). Casio JP runs a 60-minute cadence (17.8k-URL document) via the per-source cadence tier mechanism; one 15-minute timer serves all tiers.
- **Identity:** model/reference is the key; global cross-region dedup via `<manufacturer>:<reference_canonical>`. One Sentinel-specific rule, allowlisted with evidence: unhyphenated SKUs collapse a trailing `UK` regional suffix (`TW5M74100UK -> TW5M74100`, live-verified both forms exist on timex.co.uk/timex.com), colour suffixes (`TW6A01000VQ`) and hyphenated systems are never touched. Brand normalization is the SHARED `app/normalization/references.py` — the Sentinel invents no brand rules. No reference -> canonicalized-URL SHA fallback.
- **Baseline law:** a source's first successful poll is a silent per-source baseline (arming latch in `sentinel_source_state`), so first deployment can never flood Discord, a temporarily failed source neither forgets identities nor re-baselines, and `--sentinel-baseline` forces silence for a sweep (`collector_runs.is_baseline=1`).
- **Delivery:** existing `DiscordNotifier` editorial lane + `delivery_receipts` (`SENTINEL_SIGHTING`/`sentinel_sighting`, idempotent — a retry can never double-ping; failures are durably recorded, never lost). Deliberately NOT under the soak-contract experimental gate: that gate governs pipeline Events; immediate sightings are this component's entire purpose. Flood breaker: >`SENTINEL_MAX_ALERTS_PER_SWEEP` (20) in one sweep records `CAPPED` sightings + one health alert.
- **Overlap/concurrency:** `RunLockService` kernel lock (`horology_sentinel.run.lock`), overlapping firings exit `SKIPPED_OVERLAP`; the `sentinel_identities.identity_key` unique constraint is the final double-admission authority (SAVEPOINT-safe).
- **Registry wiring:** `horology_sentinel` added to `KNOWN_COLLECTORS` + `EXPECTED_CADENCE_MINUTES` (15) + `collector_registry` (`--sentinel` entrypoint, its own `_SCHEDULED_OVERRIDE` in `render_units.py` with explicit `--qualification-provenance SCHEDULED`); systemd units render automatically from the registry as usual. Migration `017_sentinel_identities` is purely additive (three tables). NOT deployed, NOT scheduled — every activation step is an explicit operator action, see `ai/handoff/SENTINEL_RUNBOOK.md`.
- **Tests:** `tests/test_sentinel.py` (18, fully offline) covering the operator's 13 required behaviours (unseen→one sighting; repeat suppressed; cross-region dedup; distinct refs both alert; price/content change silent; baseline silent; first post-baseline alerts; per-source failure isolation; representative brand identities; URL fallback; overlap/race double-admission safety; fleet delivery contract incl. receipt idempotency and DISABLED/FAILED durability; restart survival). 578 passed / 2 skipped suite-wide, Ruff clean on all touched files (remaining repo-wide Ruff debt is pre-existing, verified against baseline).
...
of which reached the user via Watch Clank. Mandatory specimen: Timex
Weekender New England, TW2Y86600/TW2Y86500.

**Autopsy (Phase 1-3):** both references were fully, correctly discovered
by `timex_products` on 2026-08-14 18:33:57 UTC — real Watch rows, real
SourceObservations, price/availability/`published_at` all correctly
parsed — but that discovery happened during the Hetzner-redeploy
force-baseline sweep, and `_record_product_transition`'s baseline guard
returns early unconditionally, before ever consulting `is_new_watch` or
anything else. Zero Events exist for either reference, ever; 11 further
observations since baseline show no transition. No specialist source ever
covered this product either, so the Layer B correlation path had nothing
to work with. **Divergence point: transition detection, single line.**
Classification: **BASELINE ABSORPTION**, compounded by a captured-but-
ignored `published_at` (Timex's own Shopify field, 2026-08-04 — ten days
before baseline).

**Control group (Phase 5-6):** three more Timex stories (Expedition
Sierra Chronograph, Peanuts x Expedition Acadia, Deepwater Meridian 300
Titanium) share the exact same mechanism — same collector, same baseline
sweep, same divergence point. A fourth ("new automatic GMT watches") is
too ambiguous to pin to one SKU (Outcome F). Citizen's Luke Skywalker
AW1910-48W is **not** the same failure: genuine live non-baseline
discovery on 2026-08-15, real Event, real confirmed Discord delivery
(`DiscordNotifier._post` only returns `True` on HTTP `<300`) two days
before the competitor's coverage — **UNRELATED CONTROL CASE**, evidence
the non-baseline path already works.

**A design correction made mid-investigation, not just a fix:** the first
version of the fix used `published_at` with a 30-day window (reusing
`availability_recent_launch_window_days` for consistency). Checked against
the real Hetzner catalogue before shipping — 240 of 1485 Timex watches
(16%) would have qualified, because Shopify's `published_at` is
frequently bulk-touched by routine catalogue maintenance across many
unrelated, genuinely old products (738 of the 1485 even share one
identical 2022 date). Tightened to a 72-hour window instead (matching the
already-trusted `specialist_freshness_window_hours` bar): only 7 of 1485
qualify, every one a genuine seconds-apart matched-sibling-pair launch
cluster, zero scatter noise. This deliberately does not retroactively
catch TW2Y86600/TW2Y86500's real 10-day gap (no historical Event was
fabricated) but does fix the general failure class for every future
baseline. Full analysis: `ai/handoff/INCIDENT_TIMEX_BASELINE_ABSORPTION.md`.

**Fix (Phase 7):** `app/services/freshness.py` gained
`classify_baseline_product_freshness()` (new function; `classify_lead_
freshness` untouched, still SpecialistLead-only) and a corrected module
docstring (its prior claim that official Events need no freshness concept
was true when written, false since `timex_products` started capturing
`published_at` without anyone revisiting it). `_record_product_transition`
now consults this — **only** for the `is_new_watch`/NEW_REFERENCE branch;
`NEW_REGION` and price/availability transitions during baseline are
completely unaffected. New setting `product_baseline_freshness_window_
hours` (default 72), independently tunable from its siblings. Every
collector without a `published_at` (Citizen, Seiko, Casio UK, ...) is
provably unaffected (dedicated regression).

**WatchBench regression (Phase 8-9):** 6 new tests — the mechanism in
isolation (fresh -> Event, stale -> silent, no-timestamp -> silent,
proven against the real 10-day TW2Y86600 gap specifically) plus a
permanent Hall-of-Shame specimen run through the real
`run_product_observation_pipeline` entrypoint with a Weekender-New-
England-shaped fixture, proving both DB persistence **and** actual
Discord delivery, plus repeat-run dedup and a no-webhook-no-crash check.
239 -> **245 tests**, Ruff clean throughout.

**Deployed to Hetzner from GitHub, no manual file copying.** Verified
post-deploy: origin/main HEAD == Hetzner deployed SHA == running image's
own `get_identity()["source_revision"]` (three-way match), DB integrity
`ok`, schema current, all 17 timers active, no stale/RUNNING rows, no
unexpected locks, Discord editorial+health webhooks still configured, no
duplicate execution, no historical replay.

**Not done, deliberately:** no new source/brand/collector, no Reddit, no
Orient Star, no GUI/architecture changes, no SSH/fail2ban changes, no DB
sync between hosts, no threshold-lowering outside the one narrow, proven,
evidenced path above, no historical Event fabrication or replay.
